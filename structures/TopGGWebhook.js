...
const initWebhook = async(bot) => {
    ...
    app.post('/dblwebhook', webhook.middleware(), (req, res) => {
        ...
        bot.db.query(`SELECT * FROM Economy WHERE userID='${req.vote.user}'`, (err, rows) => {
            if (!rows[0]) bot.db.query(`INSERT INTO Economy (userID, balance) VALUES ('${req.vote.user}', '450')`)
            else bot.db.query(`UPDATE Economy SET balance='${Number(rows[0].balance) + 450}' WHERE userID='${req.vote.user}'`)
        })
    });
