    ...
    (err, rows) => {
      ...
      message.channel
        ...
        .then (collected => {
          if (!rows[0])
            con.query (
              `INSERT INTO JoinMessages (guildID, channelID, joinmsg) VALUES ('${message.guild.id}', '${channel.id}', '${collected.first ().content}')`
            );
          else
            con.query (
              `UPDATE JoinMessages SET joinmsg = '${collected.first ().content}', channelID = '${channel.id}' WHERE guildID = '${message.guild.id}'`
            );
