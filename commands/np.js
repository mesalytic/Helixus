module.exports.run = async (bot, message, args, con) => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(`[${serverQueue.songs[0].title}](${serverQueue.songs[0].url})\n\n${bot.lang.musique.np.time} ${timeString(serverQueue.connection.dispatcher ? serverQueue.connection.dispatcher.streamTime / 1000 : 0)} / ${serverQueue.songs[0].duration_length} (${timeLeft(serverQueue.connection.dispatcher ? serverQueue.connection.dispatcher.streamTime / 1000 : 0)} ${bot.lang.musique.np.left})\n\n${bot.lang.musique.np.addedby} ${serverQueue.songs[0].requestedby}`);
