module.exports.run = async (bot, message, args, con) => {
  ...
  ment.forEach(m => {
    if (!m.bannable) {
      let str = bot.lang.mods.softban.err1.replace("${m.username}", m.user.username);
      return message.channel.send(str);
    } else {
      let id = m.id;
      m.ban({ days: 7, reason: `Helixus - Softban by ${message.author.tag}` }).then(() => {
        message.guild.members.unban(id).then(() => {
          let str = bot.lang.mods.softban.softban.replace("${m.tag}", m.user.tag);
