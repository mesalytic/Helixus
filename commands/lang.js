module.exports.run = async (bot, message, args, con) => {
  ...
  if (["en", "fr"].includes(args[0].toLowerCase())) {
    con.query(`SELECT * FROM Langs WHERE guildID='${message.guild.id}'`, (err, rows) => {
      if (!rows[0]) {
        con.query(`INSERT INTO Langs (guildID, lang) VALUES ('${message.guild.id}', '${args[0].toLowerCase()}')`);
      } else {
        con.query(`UPDATE Langs SET lang = '${args[0].toLowerCase()}' WHERE guildID='${message.guild.id}'`);
      }
