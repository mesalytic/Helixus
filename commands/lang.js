module.exports.run = async (bot, message, args, con) => {
  ...
  if (!message.member.permissions.has ('ADMINISTRATOR'))
    return message.reply (bot.lang.admin.lang.noperms);
  ...
  if (['en', 'fr', 'it'].includes (args[0].toLowerCase ())) {
    con.query (
      ...
      (err, rows) => {
        if (!rows[0])
          con.query (
            `INSERT INTO Langs (guildID, lang) VALUES ('${message.guild.id}', '${args[0].toLowerCase ()}')`
          );
        else
          con.query (
            `UPDATE Langs SET lang = '${args[0].toLowerCase ()}' WHERE guildID='${message.guild.id}'`
          );
