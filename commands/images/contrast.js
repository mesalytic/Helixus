...
module.exports = class ContrastCommand extends Command {
    ...
    async run(message, args) {
        ...
        const m = await message.channel.send(message.guild.lang.COMMANDS.CONTRAST.pleaseWait);
        
        jimp.read(url).then(img => {
            img.contrast(+1)
            .getBuffer(jimp.MIME_PNG, (err, res) => {
                m.delete();
                message.channel.send({ files: [{ attachment: new Buffer(res, 'base64'), name: "constrast.png" }] });
            })
        })        
    }
}