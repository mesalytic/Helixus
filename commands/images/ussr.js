...
module.exports = class UssrCommand extends Command {
    constructor(bot) {
        super(bot, {
            name: 'ussr',
            usage: 'ussr [url]',
    ...
    async run(message, args) {
        ...
        const m = await message.channel.send(message.guild.lang.COMMANDS.USSR.pleaseWait);

        const base = await Canvas.loadImage(`${process.cwd()}/assets/images/urss.png`);
        ...
        m.delete();
        message.channel.send({ files: [{ attachment: new Buffer(attachment, 'base64'), name: "ussr.png" }] });
    }
}