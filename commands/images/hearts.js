...
module.exports = class HeartsCommand extends Command {
    ...
    async run(message, args) {
        ...
        const m = await message.channel.send(message.guild.lang.COMMANDS.HEARTS.pleaseWait);

        const base = await Canvas.loadImage(`${process.cwd()}/assets/images/hearts.png`);
        ...
        this.drawImageWithTint(ctx, avatar, 'deeppink', 0, 0, avatar.width, avatar.height);
        ctx.drawImage(base, 0, 0, avatar.width, avatar.height);
        
        const attachment = canvas.toBuffer();

        m.delete();
        message.channel.send({ files: [{ attachment: new Buffer(attachment, 'base64'), name: "hearts.png" }] });
    }
