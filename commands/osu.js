module.exports.run = async (bot, message, args, con) => {
  ...
  const oArgs = args.slice(0).join(" ").split(" ");
  ...
  const embed = new Discord.MessageEmbed()
    .setAuthor(`osu!${oArgs[1]}`, "https://i.imgur.com/hWrw2Sv.png", "https://osu.ppy.sh/")
    .setColor("RANDOM")
    ...
    .addField(bot.lang.membres.osu.accuracy, data.accuracy ? `${Math.round(data.accuracy)}%` : "???", true)
    .addField(bot.lang.membres.osu.rank, data.pp_rank ? formatNumber(data.pp_rank) : "???", true)
    .addField("PP", data.pp_raw ? formatNumber(data.pp_raw) : "???", true)
    .addField(bot.lang.membres.osu.totalgames, data.playcount ? formatNumber(data.playcount) : "???", true)
    .addField(bot.lang.membres.osu.country, data.country || "???", true)
    .addField(bot.lang.membres.osu.rankedscore, data.ranked_score ? formatNumber(data.ranked_score) : "???", true)
    .addField(bot.lang.membres.osu.totalscore, data.total_score ? formatNumber(data.total_score) : "???", true)
    .addField("SSH", data.count_rank_ssh ? formatNumber(data.count_rank_ssh) : "???", true)
    .addField("SS", data.count_rank_ss ? formatNumber(data.count_rank_ss) : "???", true)
    .addField("SH", data.count_rank_ssh ? formatNumber(data.count_rank_sh) : "???", true)
