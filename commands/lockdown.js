module.exports.run = async (bot, message, args, con) => {
  ...
  if (!message.member.permissions.has("MANAGE_CHANNELS")) return message.channel.send(bot.lang.mods.lockdown.noperm);

  con.query(`SELECT * FROM LockdownChannels WHERE channelID='${message.channel.id}'`, (err, rows) => {
    let lockit = [];
    ...
    if (validUnlocks.includes(time)) {
      ...
      message.channel.createOverwrite(message.guild.id, {
        ...
        con.query(`DELETE FROM LockdownChannels WHERE channelID='${message.channel.id}'`);
      }).catch(error => {
        throw error;
      });
    } else {
      message.channel.createOverwrite(message.guild.id, {
        ...
        let res = str.replace("${ms(ms(time), { long:true })}", ms(ms(time), { long: true, }));
        con.query(`INSERT INTO LockdownChannels (channelID, time) VALUES ('${message.channel.id}', '${Number(new Date().getTime()) + Number(ms(time))}')`,);
