module.exports.run = async (bot, message, args, con) => {
  ...
  try {
    ...
    if (args[0] === "add") {
      if (!message.channel.permissionsFor(message.author).has("MANAGE_EMOJIS")) return message.reply(bot.lang.membres.emoji.nopermission);

      if (!args[1]) return message.reply(bot.lang.membres.emoji.add_noargs);

      if (message.attachments.first()) {
        if (message.attachments.first() && args[2]) return message.reply(bot.lang.membres.emoji.add_both);

        message.guild.emojis.create(message.attachments.first().url, args[1])
          .then(emote => {
            const str = bot.lang.membres.emoji.add_created.replace("${emote.name}", emote.name);
        ...
        if (message.attachments.first() && args[2]) return message.reply(bot.lang.membres.emoji.add_both);
        if (regex.test(args[2]) === true) return message.reply(bot.lang.membres.emoji.unicode);

        const id = parseEmoji(args[2]).id;
        if (message.guild.emojis.resolve(id)) return message.reply(bot.lang.membres.emoji.add_already);
      ...
      if (args[1].match("<")) {
        ...
        message.guild.emojis.resolve(emote)
          .delete()
          .then(() => {
            const str = bot.lang.membres.emoji.remove_success.replace("${emote.name}", emote.name);
      ...
      if (args[1].match("<")) {
        ...
        emote.edit({
          ...
          const str = bot.lang.membres.emoji.rename_success.replace("${args[2]}", args[2]);
          return message.channel.send(str);
        });
      }
      const emote = message.guild.emojis.cache.find(u => u.name === args[1]);
      ...
      if (args[0].match("<")) {
        ...
        if (emote) {
          ...
          const str = bot.lang.membres.emoji.success.replace("${name}", name).replace("${apercu}", apercu).replace("${id}", id).replace("${duration}", duration);
          message.channel.send(str, { files: [{ attachment: "https://cdn.discordapp.com/emojis/" + id, name: attName }] });
        } else {
          if (regex.test(args[0]) === true) return message.reply(bot.lang.membres.emoji.unicode);

          const id = parseEmoji(args[0]).id;
          const name = parseEmoji(args[0]).name;
          let attName;

          if (parseEmoji(args[0]).animated === true) attName = `${name}-${id}.gif`;
          else attName = `${name}-${id}.png`;

          message.channel.send({ files: [{ attachment: "https://cdn.discordapp.com/emojis/" + id, name: attName }] });
        }
      } else {
        if (args[0].match("<")) {
          const name = parseEmoji(args[0]).name;
