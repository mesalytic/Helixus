module.exports.run = async (bot, message, args, con) => {
  ...
  con.query (
    ...
    (err, modlogs) => {
      con.query (
        `SELECT * FROM WarnConfig WHERE guildID='${message.guild.id}'`,
        (err, config) => {
          con.query (
            `SELECT * FROM Cases WHERE guildID='${message.guild.id}'`,
            (err, cases) => {
              if (
                ...
                return message.reply (bot.lang.mods.warn.notperm);
              if (['remove', 'mods'].includes (args[0]))
                return message.reply ('Not available yet.');
              if (args[0] === 'list') {
                let u = message.mentions.members.first ();
                if (u) {
                  con.query (
                    `SELECT * FROM Warns WHERE memberID='${u.id}' ORDER BY number ASC`,
                    (err, rows) => {
                      if (rows[0]) {
                ...
                try {
                  ...
                  if (!reason)
                    return message.reply (bot.lang.mods.warn.noreason);
                  ...
                  con.query (
                    `SELECT * FROM Warns WHERE guildID='${message.guild.id}' AND memberID='${user.id}' ORDER BY number DESC`,
                    (err, warns) => {
                      let id;
                      if (!warns[0]) id = 1;
                      ...
                      con.query (
                        `INSERT INTO Warns (number, guildID, memberID, reason, date) VALUES ('${Number (id)}', '${message.guild.id}', '${user.id}', '${reason}', '${new Date ().getTime ()}')`
                      );
                      ...
                      if (id == kicksn) {
                        let str = bot.lang.mods.warn.warn1
                          .replace ('${guildcasenumber}', caseGuild)
                          .replace ('${user.id}', user.id)
                          .replace ('${kicksn}', kicksn)
                          .replace ('${reason}', reason);
                        let fstr = bot.lang.mods.warn.warn1_footer
                          .replace ('${mod.id}', mod.id)
                        ...
                        let str = bot.lang.mods.warn.warn2
                          .replace ('${guildcasenumber}', caseGuild)
                          .replace ('${user.id}', user.id)
                          .replace ('${bansn}', bansn)
                          .replace ('${reason}', reason);
                        let fstr = bot.lang.mods.warn.warn2_footer
                          .replace ('${mod.id}', mod.id)
                          .replace ('${mod.tag}', mod.tag);
                        ...
                        const warns = id;
                        let str = bot.lang.mods.warn.warn3
                          .replace ('${guildcasenumber}', caseGuild)
                          .replace ('${user.id}', user.id)
                          .replace ('${reason}', reason)
                          .replace ('${warns}', warns);
                        let fstr = bot.lang.mods.warn.warn3_footer
                          .replace ('${mod.id}', mod.id)
                          .replace ('${mod.tag}', mod.tag);
