module.exports.run = async (bot, message, args, con) => {
  ...
  if (!message.member.voice.channel)
    return message.channel.send(bot.lang.musique.duplicate.nochannel);
  if (!serverQueue)
    return message.channel.send(bot.lang.musique.duplicate.nomusic);
  if (!args[0] || isNaN(args[0]))
    return message.reply(bot.lang.musique.duplicate.choose);
