module.exports.run = async (bot, message, args, con) => {
  if (!message.channel.permissionsFor (message.author).has ('MANAGE_ROLES'))
    return message.channel.send (bot.lang.mods.unmute.unotperm);
  else if (!message.channel.permissionsFor (bot.user).has ('MANAGE_ROLES'))
    return message.channel.send (bot.lang.mods.unmute.bnotperm);

  let member = message.mentions.members.first ();
  if (!member) return message.channel.send (bot.lang.mods.unmute.noment);

  let muteRole = message.guild.roles.find (u => u.name === 'HelixusBeta_Muted');
  ...
  con.query (
    `SELECT * FROM MuteRoles WHERE mutedID='${member.id}' AND guildID='${message.guild.id}'`,
    (err, rows) => {
      ...
      member.roles.remove (muteRole).then (() => {
        ...
        let str = bot.lang.mods.unmute.unmute
