        ...
        message.channel.awaitMessages (filter, {max: 1}).then (collected => {
          if (collected.first ().content === 'msgChannel') {
            ...
            message.channel
              ...
              .then (collected => {
                con.query (
                  `UPDATE LevelsConfig SET lvlupMessage='${collected.first ().content}' WHERE guildID='${message.guild.id}'`
                );
        ...
        con.query (
          ...
          (err, lrows) => {
            ...
            if (args[1] === 'xp') {
              if (args[2] === 'add') {
                con.query (
                  `UPDATE Levels SET points='${Number (lrows[0].points) + Number (args[4])}' WHERE id='${message.guild.id}-${u.id}'`
                );
                message.channel.send (
              ...
              if (args[2] === 'add') {
                con.query (
                  `UPDATE Levels SET level='${Number (lrows[0].level) + Number (args[4])}' WHERE id='${message.guild.id}-${u.id}'`
                );
                message.channel.send (
                  bot.lang.levels.levels.modify_xpadded
                    .replace ('${args[3]}', args[4])
