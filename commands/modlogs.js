module.exports.run = async (bot, message, args, con) => {
  ...
  con.query(`SELECT * FROM ModlogsChannel WHERE guildID='${message.guild.id}'`, (err, rows) => {
    ...
    if (args[0] === "reset") {
      if (rows[0]) {
        con.query(`DELETE FROM ModlogsChannel WHERE guildID='${message.guild.id}'`);
    ...
    if (!rows[0]) {
      con.query(`INSERT INTO ModlogsChannel (guildID, channelID) VALUES ('${message.guild.id}', '${chan.id}')`);
    } else { con.query(`UPDATE ModlogsChannel SET channelID='${chan.id}'`); }
    message.channel.send(bot.lang.admin.modlogs.success.replace("${chan}", chan));
  });
};
module.exports.help = {
  name: "modlogs",
