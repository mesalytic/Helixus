module.exports.run = async (bot, message, args, con) => {
  ...
  const banned_tags = ["child", "childs", "childporn", "children", "youngs", "young", "loli", "shota", "cub"];
  ...
  fetch(url).then(res => res.json()).then(json => {
    if (json.length > 0) {
      ...
      message.channel.send(`${bot.lang.nsfw.yandere.real_size} <${post.file_url}>`, { files: [post.preview_url], });
    } else {
