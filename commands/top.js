          ...
          con.query (
            ...
            (err, top) => {
              ...
              for (let i = 0; i < rows.length; i++) {
                ...
                if (i == 0) {
                  if (!u)
                    msgtop += `[${i + 1}] :first_place: **${bot.lang.levels.top.notfound}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                  else
                    msgtop += `[${i + 1}] :first_place: **${u.tag}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                } else if (i == 1) {
                  if (!u)
                    msgtop += `[${i + 1}] :second_place: **${bot.lang.levels.top.notfound}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                  else
                    msgtop += `[${i + 1}] :second_place: **${u.tag}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                } else if (i == 2) {
                  if (!u)
                    msgtop += `[${i + 1}] :third_place: **${bot.lang.levels.top.notfound}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                  else
                    msgtop += `[${i + 1}] :third_place: **${u.tag}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                } else {
                  if (!u) {
                    msgtop += `[${i + 1}] :military_medal: **${bot.lang.levels.top.notfound}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP) \n`;
                  } else
                    msgtop += `[${i + 1}] :military_medal: **${u.tag}** - Level ${rows[i].level} | (${rows[i].points}/${diff} XP)\n`;
                }
              }
                ...
                (err, top) => {
                  ...
                  for (let k = 0; k < top.length; k++) {
                    ...
                    if (!user)
                      ...
                      topall += `[${k + 1}] ${user.tag} - Level ${top[k].level} | (${top[k].points}/${topDiff} XP)\n`;
                  }
