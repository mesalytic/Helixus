module.exports.run = async (bot, message, args, con) => {
  ...
  con.query (
    `SELECT * FROM LeaveMessages WHERE guildID=${message.guild.id}`,
    (err, rows) => {
      if (!message.member.permissions.has ('MANAGE_CHANNELS'))
        return message.reply (bot.lang.admin.leavemsg.noperms);
      ...
      message.channel
        ...
        .then (collected => {
          if (!rows[0])
            con.query (
              `INSERT INTO LeaveMessages (guildID, channelID, leavemsg) VALUES ('${message.guild.id}', '${channel.id}', '${collected.first ().content}')`
            );
          else
            con.query (
              `UPDATE LeaveMessages SET leavemsg = ${collected.first ().content}, channelID = ${channel.id} WHERE guildID = ${message.guild.id}`
            );
